...
if __name__ == "__main__":
    ...
    length=len(log_rate)
    result = fit_levy(log_rate)
    print('拟合参数为：')
    print('alpha={:.2f}, beta={:.2f}, mu_0={:.2f}, sigma={:.2f}, neglog={:.2f}'.format(*result))
    alpha, beta, mu, sigma = result[0], result[1], result[2], result[3]       

    rx=np.linspace(-4.0,4.0, 100)
    sns.plt.plot(rx/80, levy(rx, result[0], result[1], result[2], result[3])*length, 'k-', color='green', lw=2, label='levy stable pdf')
    '''sns.plt.hist(log_rate, bins =100, color = 'r',alpha=0.5,rwidth= 0.9, normed=True)'''
